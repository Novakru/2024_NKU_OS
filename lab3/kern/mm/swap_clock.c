...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
    ...
    while (1) {
		...
		list_entry_t* next_ptr = list_prev(curr_ptr);
		curr_ptr = (next_ptr == head) ? list_prev(next_ptr) : next_ptr;
        // 获取当前页面对应的Page结构指针
		struct Page* currpg = le2page(curr_ptr, pra_page_link);
